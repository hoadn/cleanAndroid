package com.cssweb.android.domain;

public class Road {
	private String name;
	private String zixun;
	private String wsyyt;
	private String webcall;
	private String mncg;
	private String validation;
	private String hqyj;
	
	private JYWG jywg;
	private HQFWQ hqfwq;
	public String getName() {
		return name;
	}
	public void setName(String name) {
		this.name = name;
	}
	public String getZixun() {
		return zixun;
	}
	public void setZixun(String zixun) {
		this.zixun = zixun;
	}
	public String getWsyyt() {
		return wsyyt;
	}
	public void setWsyyt(String wsyyt) {
		this.wsyyt = wsyyt;
	}
	public String getWebcall() {
		return webcall;
	}
	public void setWebcall(String webcall) {
		this.webcall = webcall;
	}
	public String getMncg() {
		return mncg;
	}
	public void setMncg(String mncg) {
		this.mncg = mncg;
	}
	public String getValidation() {
		return validation;
	}
	public void setValidation(String validation) {
		this.validation = validation;
	}
	public String getHqyj() {
		return hqyj;
	}
	public void setHqyj(String hqyj) {
		this.hqyj = hqyj;
	}
	public JYWG getJywg() {
		return jywg;
	}
	public void setJywg(JYWG jywg) {
		this.jywg = jywg;
	}
	public HQFWQ getHqfwq() {
		return hqfwq;
	}
	public void setHqfwq(HQFWQ hqfwq) {
		this.hqfwq = hqfwq;
	}
	
	
}
